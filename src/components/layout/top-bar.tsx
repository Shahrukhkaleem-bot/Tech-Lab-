...
export function TopBar({ tenant, settings }: { tenant: Tenant; settings: StoreSettings }) {
  ...
  return (
    <div className="bg-secondary text-xs text-secondary-foreground">
      <div className="container-page flex h-10 items-center justify-between gap-4 sm:h-9">
        <ul className="flex min-w-0 items-center gap-4 sm:gap-5">
          {phone ? (
            <li>
              <a href={`tel:${phone.replace(/\s+/g, "")}`} className="flex items-center gap-1.5 py-2 hover:underline">
        ...
        <SocialLinks links={tenant.social} className="-mr-2 shrink-0" itemClassName="size-9 sm:size-7" />
      </div>
    </div>
  )
}
