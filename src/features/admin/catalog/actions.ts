...
export async function saveBannerAction(bannerId: string | null, input: unknown): Promise<ActionResult<{ id: string }>> {
  return adminAction("banners.save", "manageContent", async (ctx) => {
    ...
    const row = {
      ...
      desktop_image_width: v.desktopImageWidth ?? null,
      desktop_image_height: v.desktopImageHeight ?? null,
      mobile_image_width: v.mobileImageUrl ? (v.mobileImageWidth ?? null) : null,
      mobile_image_height: v.mobileImageUrl ? (v.mobileImageHeight ?? null) : null,
      show_text: v.showText,
      display_order: v.displayOrder,
      is_active: v.isActive,
